...
export default function PostDetail() {
    ...
    (async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/posts/${query.id}`
        );
  ...
  async function submit(e) {
    e.preventDefault();
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ postId: Number(query.id), content: comment }),
        credentials: 'include',
      });
  ...
  return (
    <Layout>
      ...
      <article className="prose max-w-none mt-4">{post.content}</article>
